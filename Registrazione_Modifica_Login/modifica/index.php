...
<body class="text-center" background="../media/sfondo.webp">
    ...
    <form action="modifica.php" method="POST" class="form-update" name="myForm">
        ...
        <input type="text" name="nome" class="form-control" value="<?php echo $_SESSION["nome"]; ?>" pattern="^[a-zA-Z]+$" required />
        <input type="text" name="cognome" class="form-control margine" value="<?php echo $_SESSION["cognome"]; ?>" pattern="^[a-zA-Z]+$" required />

        <p class="Explain">Cellulare:</p>
        <input type="text" name="cell" class="form-control margine" value="<?php echo $_SESSION["cellulare"];  ?>" maxlength="10" pattern="[0-9]{10}$" required />
        
        <p class="Explain">Codice Fiscale:</p>
        <input type="text" name="cf" class="form-control margine" value="<?php echo $_SESSION["cf"];  ?>" pattern="^[a-zA-Z]{6}[0-9]{2}[a-zA-Z][0-9]{2}[a-zA-Z][0-9]{3}[a-zA-Z]$" required />

        <p class="Explain">Email e Password:</p>
        <input type="email" name="email" class="form-control margine" value="<?php echo $_SESSION["email"];  ?>" required />
        <div>
            <input type="password" class="form-control margine" name="pswd" id="pass" value="<?php echo $_SESSION["password"];  ?>" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*_=+-.]).{8,16}$" required />
            <i class="bi bi-eye-slash" id="togglePassword"></i>
        </div>

        <p class="Explain">Regione:</p>
        <input class="box margine" id="boxRegione" list="Regione" name="regione" value="<?php echo $_SESSION["regione"]; ?>">
        <datalist id="Regione">
        ...
        <input type="text" class="form-control" name="città" value="<?php echo $_SESSION["città"];  ?>" pattern="^[a-zA-Z]+$" required>
        <input type="text" class="form-control margine" name="via" value="<?php echo $_SESSION["via"];  ?>" pattern="^[a-zA-Z\s]+[0-9]*" required>



        <div id="divCap" class="mb-3">
            <label for="cap" class="Explain">CAP (5 cifre):</label>
            <input type="text" name="cap" maxlength=5 size=5 value="<?php echo $_SESSION["cap"];  ?>" pattern="^[0-9]{5}" required />
        </div>
